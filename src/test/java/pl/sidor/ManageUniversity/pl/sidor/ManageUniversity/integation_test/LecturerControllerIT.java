...
public class LecturerControllerIT {
    ...
    public void test_should_throw_exception_when_id_is_incorrect() throws Exception {
        ...
        // expected
        mockMvc.perform(get("lecturer/{id}", id))
                .andDo(print())
    ...
    public void should_save_lecturer() throws Exception {
        ...
        // when
         mockMvc.perform(post("/lecturer/save")
    ...
    public void test_should_throw_exception_when_lecturer_null() throws Exception {
        ...
        // then
        mockMvc.perform(post("lecturer/save").content(objectMapper.writeValueAsString(lecturer))
    ...
    public void should_delete_lecturer() throws Exception {
        ...
        // when
        mockMvc.perform(delete("/lecturer/{id}", id))
                .andExpect(status().isOk())
                .andReturn();
    ...
    public void should_update_lecturer() throws Exception {
        ...
        // when
        mockMvc.perform(post("/lecturer/update").content(objectMapper.writeValueAsString(updateLecturer))
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk())
                .andReturn();
    ...
    public void testShouldFindLecturerDTO() throws Exception {
        ...
        // expected
        mockMvc.perform(get("/lecturer/lecturerDTO/{id}", prepareLecturer().getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name", Matchers.is("Jan")))
                .andExpect(jsonPath("$.lastName", Matchers.is("Kowalski")))
                .andExpect(jsonPath("$.email", Matchers.is("kowalski@wp.pl")))
    ...
    public void test_should_throw_exception_when_lecturerDto_incorrect_id() throws Exception {
        ...
        //  when
        MvcResult result = mockMvc.perform(get("lecturer/lecturerDTO/{id}", id))
                .andDo(print())
                .andExpect(status().isNotFound())
                .andReturn();
    ...
    public void testShouldFindScheduleForLecturer() throws Exception {
        ...
        Subject subject = prepareSubject(lecturer);
        Schedule schedule = prepareSchedule(subject);
        ...
        when(scheduleRepo.findBySubjects(subject)).thenReturn(Collections.singletonList(schedule));

        // expected
        mockMvc.perform(get("/lecturer/findSchedule").content(objectMapper.writeValueAsString(request))
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk())
                .andReturn();
    }
